...
def train(train_loader, model, criterion, optimizer, epoch, half=True, print_freq=50, double=False, adv=None):
    ...
    for i, (input_data, target) in enumerate(train_loader):
        ...
        # Adv. training
        if adv:
            input_var = adv(input_var, target_var)
...
def train_inverse(train_loader, model, attack, optimizer, epoch, half=True, print_freq=50, double=False, adv=None):
    """
        Run one train epoch
    """
    batch_time = AverageMeter()
    ...
    for i, (input_data, target) in enumerate(train_loader):
        ...
        noise_batch_size = int(batch_size)
        noises = torch.rand([noise_batch_size, 3, 32, 32])
        noisy_label = torch.randint(0, 10, [noise_batch_size])
        noisy_images = attack(noises, noisy_label).view(noise_batch_size, 3, 32, 32)
        target_var = (torch.ones(noise_batch_size, 10) / 10).cuda()
        output2 = model(noisy_images)
        loss2 = nn.KLDivLoss()(nn.LogSoftmax()(output2), target_var)
        loss += 0.1 * loss2
...
def validate(val_loader, model, criterion, adv=None, half=True, print_freq=50, double=False):
    """
    Run evaluation
    """
    batch_time = AverageMeter()
